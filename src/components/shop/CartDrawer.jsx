...
export default function CartDrawer({ open, onClose }) {
  ...
  const handleCheckout = () => {
    onClose();
    if (!isAuthenticated) {
      navigate('/login', { state: { from: '/checkout', message: 'Sign in to complete your purchase' } });
    } else {
      navigate('/checkout');
    }
  };
                ...
                <div className="px-5 py-4 space-y-3">
                  <AnimatePresence>
                    {items.map((item) => (
                      ...
                      >
                        <Link to={`/shop/${item.id}`} onClick={onClose} className="shrink-0">
                        ...
                        <div className="flex flex-col items-end justify-between">
                          ...
                          <div className="flex items-center gap-0 rounded-lg border border-border/50 overflow-hidden">
                            ...
                            <span className="w-7 text-center font-bold text-sm text-foreground bg-card">{item.quantity}</span>
                            <button
                              onClick={() => updateQty(item.id, item.quantity + 1)}
                              className="w-7 h-7 flex items-center justify-center bg-secondary hover:bg-secondary/80 text-foreground"
                            >
            ...
            {items.length > 0 && (
              <div className="border-t border-border/40 px-6 py-5 space-y-4">
                ...
                <motion.button
                  ...
                  className="w-full flex items-center justify-center gap-2.5 py-4 rounded-xl bg-primary text-primary-foreground font-display font-bold text-base hover:shadow-[0_0_28px_rgba(34,197,94,0.4)] transition-all"
                >
                  Proceed to Checkout
                ...
                <Link
                  to="/shop"
                  onClick={onClose}
                  className="block w-full text-center py-2 text-xs font-body text-muted-foreground hover:text-foreground transition-colors"
                >
                  Continue shopping
                </Link>
              </div>
